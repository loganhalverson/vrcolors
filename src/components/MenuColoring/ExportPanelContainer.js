...
export const ExportPanelContainer = () => {
	...
	const copyContent = async (input) => {
		try {
			...
			toast.error(
				`${error.name}: Unable to write to clipboard. Enable permissions and try again.`,
				toastStyles.error
			);
		}
	};
				...
				<>
					<div className="fixed left-0 top-0 z-10 w-full">
						<div className="flex min-h-screen items-center justify-center">
							{/* Transparent Overlay */}
							<div
								className="absolute left-0 top-0 min-h-screen w-full bg-black opacity-80"
								onClick={() => setVisible(!visible)}
							/>

							<div className="z-10 rounded-xl bg-neutral-900">
								{/* Header Container */}
								<div className="relative rounded-xl rounded-b-none border-b border-neutral-700 bg-[#0D3537] py-4 text-center font-semibold text-white">
									<p className="text-2xl ">Export Palette</p>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										className="absolute left-2 top-0 h-8 translate-y-1/2 cursor-pointer"
										onClick={() => setVisible(!visible)}
										viewBox="0 0 24 24"
									>
										<path
								...
								<div className="flex flex-row items-center justify-center text-center">
									<div
										onClick={() => exportLink()}
										className="group flex w-1/2 flex-none cursor-pointer flex-col  items-center p-4 px-16 py-16 text-white focus:bg-slate-800"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											className="w-24 -rotate-45 transition duration-300 group-hover:scale-105 group-hover:text-teal-500"
											viewBox="0 0 24 24"
										>
											<path
												fill="currentColor"
										...
										<p className="mt-2 text-2xl font-semibold tracking-wide transition duration-300 group-hover:text-teal-500">
											Page Link
										</p>
										<p className="leading-8 tracking-tight text-gray-300 ">
									...
									<div
										...
										className="group flex w-1/2 flex-none cursor-pointer flex-col items-center p-4 px-16 py-16 text-white  hover:text-teal-400 focus:bg-slate-800"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											className="w-24 transition duration-300 group-hover:scale-105 group-hover:text-teal-500"
											viewBox="0 0 24 24"
										>
											<path
												fill="currentColor"
										...
										<p className="mt-4 max-w-md text-clip tracking-tight text-gray-600">
											#F4EBD0,#B68D40,#0F0F10...
										</p>
									</div>
								</div>
							</div>
